...
uint64 sys_mmap(void)
{
  ...
  // so addr here is ignored and a new unmapped va region is found to map the file
  ...
  for(int i = 0; i < NVMA; i ++) {
    ...
    if(vv->valid == 0) {
      ...
      vaend = PGROUNDDOWN(vv->vastart);  // 更新最低位置
    }
  }
  ...
  v->vastart = vaend - sz;  // vaend用来确定起始位置
  v->sz = sz;
  v->prot = prot;
  v->flags = flags;
  ...
  filedup(v->f);  // 给文件增加引用计数

  return v->vastart;
}

// 函数放的顺序也很重要，必须放munmap的上面
...
uint64 sys_munmap(void)
{
  ...
  if(argaddr(0, &addr) < 0 || argaddr(1, &sz) < 0 || sz == 0) return -1;
...
int vmatrylazytouch(uint64 va) {
  struct proc *p = myproc();
  struct vma *v = findvma(p, va); // 判断是不是vma懒分配
  if(v == 0) {
    return 0;
  }
